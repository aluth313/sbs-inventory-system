...
<body  style="margin-top: -30px; margin-left: -20px;margin-right: -20px;">
	<table width="100%"> 
		...
		<tr>
			<td width="30%"><p style="margin-top:-20px;font-size: 13px; ">{{$profil->company_address}}
					...
					Bpk/Ibu {{$tt->customer_name}}<br>
					Di {{$tt->customer_address}}
				</td>
			<td  style="font-size: 14px;" colspan="5" width="30%" rowspan="2">
				Invoice No &nbsp;&nbsp;: {{$tt->fak_no}}
				<br>
				DO Kami &nbsp;&nbsp;&nbsp;&nbsp;: {{$tt->do_no}}
				<br>
				PO No.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;      : {{$tt->po_no}}
				<br>
				Tujuan.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;     : {{substr($tt->customer_address, 0, 30)}}
				<br>
				Tanggal&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;     : {{date("d-M-Y", strtotime($tt->created_at))}}
			</td>
	...
	<?php }?>
		...
		<tr>
			<th rowspan="3" colspan="4"> * Terbilang:  {{\App\Helpers\Terbilang::kekata($gtotal - $tt->dp)}} Rupiah.</th>
		...
		<!-- <tr>
			...
			<th style="text-align: right;">({{ number_format($tt->dp) }})</th>
		</tr>
		<tr>
			<th style="text-align: right;">Belum Dbyr Rp. </th>
			<th style="text-align: right;">{{ number_format( $gtotal - $tt->dp) }}</th>
		</tr> -->
	</table>
