  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ url('custpay') }}">Pembayaran Service</a></li>
    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <button onclick="findService()" id="cari_service" class="btn btn-info"><i class="fa fa-search"></i> Cari Service</button>
          ...
          <div class="row">
            <div class="col-md-2">
              <div class="form-group">
                ...
                <input id="no_service" name="no_service" type="text" class="form-control" readonly >
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
        ...
        <div class="box-body">
          <div class="table-responsive">
            <table id="custpay-table" class="table table-striped table-bordered table-hover">
              <thead>
                  <tr>
                      <th width="2%">ID</th>
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
              <div class="modal-header">
                  ...
                  <h3 class="modal-title">Service Belum Dibayar</h3>
              </div>
                ...
                <table style="width: 100%" id="list-table" class="table table-bordered table-striped">
                  <thead>
                    <tr>
                      ...
                      <th width="20%">No Service</th>
                      <th width="*">Customer/Item</th>
...
<script type="text/javascript">
          ...
          var table = $('#custpay-table').DataTable({
              processing:true,
              serverSide:true,
              order: [[ 0, "desc" ]],
              ajax: "{{ route('api.custpay') }}",
              columns: 
              [
                {data:'id', name: 'id'},
          ...
          function chooseService(id){
              $.ajax({
                  url : "{{ url('pilih_service') }}"+"/"+id,
                  type : "GET",
                  dataType : "JSON",
                  success : function(data){
                    // console.log(data);
                    $('#no_service').val(data[0].invoice);
          ...
          function savePembayaran(){
              ...
              if(pembayaran == ''){
                  ...
                  $.ajax({
                      url : "{{ url('simpan_pembayaran_service') }}",
                      type : "POST",
                      data : {noservice:noservice, idcustomer:idcustomer, namacustomer:namacustomer, pembayaran:pembayaran, sisa:sisa, ket:ket, _token:_token},
                      success : function(data){
                          reloadTable();
                          clearForm();
          ...
          function updatePembayaran(){
              ...
              if(pembayaran == ''){
                  ...
                  alert("Belum Ada Service Dipilih...!");
              }
              else{
                  $.ajax({
                      url : "{{ url('update_pembayaran_service') }}",
                      type : "POST",
                      data : {id_bayar:nopembayaran, noservice:noservice, idcustomer:idcustomer, namacustomer:namacustomer, pembayaran:pembayaran, sisa:sisa, ket:ket, _token:_token},
                      success : function(data){
                          reloadTable();
                          clearForm();
          ...
          function editData(id){
              $.ajax({
                  url : "{{url('get_data_payment_service')}}"+"/"+id,
                  type :"GET",
                  dataType : "JSON",
                  success :function(data){
                      $('#no_service').val(data[0].service_id);
          ...
          function confirmDeleteData(id){
              var _token = $('meta[name="csrf-token"]').attr('content');
              $.ajax({
                  url : "{{ url('hapus_pembayaran_service') }}",
                  type : "POST",
          ...
          $('#custpay-table').on('click', '#printData', function(){
              ...
              $.ajax({
                  url     : "{{ url('cetak_pembayaran_service')}}"+"/"+id+"/"+invoice,
                  type    : "GET",
                  success : function(data){
                    $('#loadingProgress').hide();
                    window.open("{{ url('cetak_pembayaran_service')}}"+"/"+id+"/"+invoice, "Nota Pembayaran ", "height=900, width=1000, scrollbars=yes");
                      reloadTable();
                  },
                  error: function(){
                    $('#loadingProgress').hide();
          ...
          function reloadTable() {
            $("#custpay-table").DataTable().ajax.reload(null,false);
