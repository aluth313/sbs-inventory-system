  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="javascript:void(0);">Edit Produksi</a></li>
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <div class="row">
          <div class="col-md-12">
              <h3 class="pull-left"><u> No Produksi :{{ $purchase->production_number }} </u></h3>
              <input type="hidden" id="invoice" value="{{$purchase->production_number}}">
          </div>
            ...
            <div class="form-group">
              ...
              <select style="width:100%" id="customer" name="customer" class="form-control" required>
                  ...
                  @foreach($customer as $key)
                    <option <?php if($purchase->id_customer == $key->id){ echo 'selected';} ;?> value="{{ $key->id }}">{{ $key->customer_name }}</option>
                  @endforeach;
              </select>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group">
              <label>Grade</label>
              <input value="{{ $purchase->grade }}" type="text" class="form-control" id="grade" name="grade" required>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group">
              <label>Colour</label>
              <input value="{{ $purchase->colour }}" type="text" class="form-control" id="colour" name="colour" required>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group">
              <label>Hardness</label>
              <input value="{{ $purchase->hardness }}" type="text" class="form-control" id="hardness" name="hardness" required>
            </div>
          </div>
        ...
        <div class="box-body">
          <div class="table-responsive">
            ...
            <div class="col-md-2">
                ...
                <button onclick="simpanProduksi()" class="btn btn-sm btn-primary form-control"><i class="fa fa-save"></i> Update</button>
...
<script type="text/javascript">
          ...
          function simpanProduksi(){
            ...
            if(popup==true){
                var production_number = $("#invoice").val();
                ...
                $.ajax({
                    url : "{{ url('update_produksi') }}",
                    type : "POST",
                    data : { 'deskripsi':deskripsi, 'production_number': production_number, 'customer':customer, 'grade':grade, 'colour':colour, 'hardness':hardness, 'batch':batch, '_token':csrf_token  },
                    success:function(data){
                          alert('Produksi Diupdate...');
